...
public class FolderDaoImpl implements FolderDao, Constants {
	...
	public InOutParameters insert(Folder folder) {
		...
		inOutParameters.setRequestPayloadSize((GraphLayout.parseInstance(folder).totalSize()) / 1024.0);
		try {
			f = folderRepository.insert(folder);
			inOutParameters.setFolder(f);
	...
	public InOutParameters findById(String id, String tenantId) {
		...
		Double reqSize = (GraphLayout.parseInstance(query).totalSize()) / 1024.0;
		System.out.println(reqSize);
		...
		Double resSize = 0.0;
		if (folder != null) {
			resSize = (GraphLayout.parseInstance(folder).totalSize()) / 1024.0;
			inOutParams.setResponsePayloadSize((GraphLayout.parseInstance(folder).totalSize()) / 1024.0);
		}
		inOutParams.setFolder(folder);
		System.out.println(resSize);
		/*
		 * folder.setRequestPayloadSize((GraphLayout.parseInstance(query).totalSize()) /
		 * 1024.0);
		 * folder.setResponsePayloadSize((GraphLayout.parseInstance(folder).totalSize())
		 * / 1024.0);
		 */
		...
		try {
			...
			System.out.println("Error in saving log in Logging Service");
		}
		return inOutParams;
	}
	 ...
	 * 
	 * 
	 * @Override public Folder findById(Query query) {
	 * 
	 * // query.addCriteria(Criteria.where("folderType").is("folder"));
	 * 
	 * Double reqSize = (GraphLayout.parseInstance(query).totalSize()) / 1024.0;
	 ...
	 * long startTime = System.nanoTime();
	 * 
	 * Folder folder = null; try { folder = (Folder) mongoTemplate.findOne(query,
	 * Folder.class); } catch (UncategorizedMongoDbException ex) { } finally { if
	 * (folder == null) {
	 * logger.debug("Exception thrown---------retrying action.... "); folder =
	 * (Folder) mongoTemplate.findOne(query, Folder.class); } logger.debug(" " +
	 * folder); }
	 * 
	 * logger.debug("Found folder : " + folder);
	 * 
	 * Double resSize = (GraphLayout.parseInstance(folder).totalSize()) / 1024.0;
	 * System.out.println(resSize);
	 * 
	 * long endTime = System.nanoTime(); try {
	 * 
	 * } catch (Exception e) {
	 * 
	 * System.out.println("Error in saving log in Logging Service"); }
	...
	public InOutParameters findByParentFolderId(String id, String tenantId) {
		...
		try {
			...
			inOutParams.setResponsePayloadSize((GraphLayout.parseInstance(folders).totalSize()) / 1024.0);
			inOutParams.setFolders(folders);
		} catch (UncategorizedMongoDbException ex) {
			// TODO Auto-generated catch block
			logger.error(ex.getMessage());
		} finally {
	...
	public InOutParameters findAllFolders(Map<String, String[]> paramMap, String tenantId) {
		...
		try {
			...
			folders = mongoTemplate.find(query, Folder.class);
			System.out.println((GraphLayout.parseInstance(query).totalSize()) / 1024.0);
		} catch (UncategorizedMongoDbException ex) {
			// TODO Auto-generated catch block
			logger.error(ex.getMessage());
		} finally {
		...
		Double resSize = 0.0;
		if (folders != null)
			resSize = (GraphLayout.parseInstance(folders).totalSize()) / 1024.0;
		inOutParams.setResponsePayloadSize(resSize);
		// System.out.println((GraphLayout.parseInstance(folders).totalSize()) /
		// 1024.0);
		inOutParams.setFolders(folders);
		return inOutParams;
	}
	...
	public InOutParameters findAllFoldersByPage(Map<String, String[]> paramMap, String tenantId, int Pno) {
		...
		inOutParameters.setFolders(folders);
		return inOutParameters;
	}
	...
	public InOutParameters findAndModify(String id, String updateFolderParams, Long version, String tenantId) {
		...
		try {
			...
			inOutParameters.setResponsePayloadSize((GraphLayout.parseInstance(updatedFolder).totalSize()) / 1024.0);
			inOutParameters.setFolder(updatedFolder);
		} catch (UncategorizedMongoDbException ex) {
			// TODO Auto-generated catch block
			logger.error(ex.getMessage());
		} finally {
	...
	private Map<String, String> handleMetadata(String updateFolderParams, String id, String tenantId) {
		...
		InOutParameters folder_temp = findById(id, tenantId);
		if (folder_temp.getFolder().getMetadata() != null) {// If already metadata exists, append new map
			Map<String, String> Existing_metamap = folder_temp.getFolder().getMetadata();
	...
	public InOutParameters updateParentFolderId(String id, String targetId, Long version, String tenantId) {
		...
		try {
			movedFolder = (Folder) mongoTemplate.findAndModify(query, update,
					new FindAndModifyOptions().returnNew(true), Folder.class);
			inOutParams.setResponsePayloadSize((GraphLayout.parseInstance(movedFolder).totalSize()) / 1024.0);
			inOutParams.setFolder(movedFolder);
		} catch (UncategorizedMongoDbException ex) {
			// TODO Auto-generated catch block
			logger.error(ex.getMessage());
		} finally {
