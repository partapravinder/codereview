...
public class HybridRoleRightUserGroupAssociationDaoImpl implements HybridRoleRightUserGroupAssociationDao {
	...
	@Override
	public InOutParameters save(UserGroupAssociation userGroupAssociation) {
		InOutParameters inOutParameters = new InOutParameters();
		UserGroupAssociation userGroupAssociationResult = userGroupAssociationRepository.save(userGroupAssociation);
		inOutParameters.setRequestPayloadSize((GraphLayout.parseInstance(userGroupAssociation).totalSize()) / 1024.0);
		inOutParameters
				.setResponsePayloadSize((GraphLayout.parseInstance(userGroupAssociationResult).totalSize()) / 1024.0);
		inOutParameters.setUserGroupAssociation(userGroupAssociationResult);
	...
	public InOutParameters findById(String id, String tenantId) {
		...
		if (userGroupAssociation != null) {
			resSize = (GraphLayout.parseInstance(userGroupAssociation).totalSize()) / 1024.0;
		}
		inOutParameters.setResponsePayloadSize(resSize);
		inOutParameters.setUserGroupAssociation(userGroupAssociation);
		return inOutParameters;
	}
	...
	public InOutParameters findAndRemoveById(String id, String tenantId) {
		...
		userGroupAssociation.getUserIds().forEach(user -> {
			...
			InOutParameters inOutParams = hybridRoleRightGroupUserAssociationDao.findById(user, tenantId);
	...
	public UserGroupAssociation findAndModifyByGroup(String groupId, Map<String, Object> updateParams,
		...
		UserGroupAssociation userGroupAssociation1 = findById(groupId, tenantId).getUserGroupAssociation();
		...
		if (updateduserids.size() > 0) {
			for (Object user : updateduserids) {
				long sT = System.nanoTime();
				InOutParameters inOutParams = hybridRoleRightGroupUserAssociationDao.findById(user.toString(),
						tenantId);
				long eT = System.nanoTime();
				wrapperServiceImpl.callLoggingService(tenantId, null, "CosmosDB", sT, eT,
						inOutParams.getRequestPayloadSize(), inOutParams.getResponsePayloadSize(), "GET",
	...
	public InOutParameters findAll(Map<String, String[]> paramMap, String tenantId) {
		...
		Double resSize = 0.0;
		List<UserGroupAssociation> userGroupAssociationList = mongoTemplate.find(query, UserGroupAssociation.class);
		inOutParameters.setRequestPayloadSize((GraphLayout.parseInstance(query).totalSize()) / 1024.0);
		if (userGroupAssociationList != null) {
			resSize = (GraphLayout.parseInstance(userGroupAssociationList).totalSize()) / 1024.0;
		}
		inOutParameters.setResponsePayloadSize(resSize);
		inOutParameters.setUserGroupAssociationList(userGroupAssociationList);
		return inOutParameters;
	}
	/*
