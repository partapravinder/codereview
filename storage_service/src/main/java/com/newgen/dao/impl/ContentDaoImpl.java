...
//	public static final String CHECKEDOUTTIME_PARAM = "checkedOutTime";
	...
	@SuppressWarnings("unchecked")
	public List<Content> findAllContentsBySearchString(Map<String, Set<String>> paramMap, String tenantId, boolean ftsEnabled) throws ParseException {
		...
		for (Map.Entry<String, Set<String>> entry : paramMap.entrySet()) {
			if ("creationDateTime".equalsIgnoreCase(entry.getKey())) {
				if(entry.getValue().size() > 0) {
					...
					Date from = sdf.parse(itr.next());
					Date to = sdf.parse(itr.next());
					Date tmp = from;
					if(from.after(to)) {
						from = to;
						to = tmp;
	...
	public List<Content> findAllContentByKeyword(Map<String, Set<String>> paramMap, String tenantId, boolean ftsEnabled) throws ParseException {
		...
		for (Map.Entry<String, Set<String>> entry : paramMap.entrySet()) {
			if ("creationDateTime".equalsIgnoreCase(entry.getKey())) {
				...
				if(entry.getValue().size() > 0) {
					String value = entry.getValue().iterator().next();
					if(!StringUtils.isEmpty(value)) {
						andCriteriaList.add(Criteria.where("metadata.keyword").regex(value,"i"));
	...
	public List<Content> findAllContentByDataClass(Map<String, Set<String>> paramMap, String tenantId, boolean ftsEnabled) throws ParseException {
		...
		for (Map.Entry<String, Set<String>> entry : paramMap.entrySet()) {
			if ("creationDateTime".equalsIgnoreCase(entry.getKey())) {
				...
				if(entry.getValue().size() > 0) {
					String value = entry.getValue().iterator().next();
					if(!StringUtils.isEmpty(value)) {
						andCriteriaList.add(Criteria.where("dataclass.dataclassname").regex(value,"i"));
	...
	@SuppressWarnings("unchecked")
	public List<Folder> findAllFolders(Map<String, Set<String>> paramMap,String tenantId) throws ParseException {
		boolean nameFilterExists = false;
		Query query = new Query();
		List<Criteria> andCriteriaList = new ArrayList<Criteria>();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
