...
public class WrapperServiceImpl implements WrapperService {
	...
	public String readUserRightsById(String id, String userId, String tenantId, String objectType)
		...
		} else if (objectType.equals("content")) {
		...
		if (folderPrivilege.toUpperCase().equals("SHARED")) {
			...
			if (arrStr != null) {
				...
				ResponseEntity<String> response2 = restTemplate.exchange(groupprofileUrl + "/groupprofile?" + str, HttpMethod.GET,
						entity, String.class);
			...
			if (ownerId.equals(userId)) {
				...
				exceptionThrower.throwUserIsNotOwnerOfThisObject();
				// throw exception
			}
			...
			if (response1!=null  && response1.getStatusCode().equals(HttpStatus.OK)) {
				...
				if (str != "") {
					...
					if (response.getStatusCode().equals(HttpStatus.OK)) {
						...
						IntStream.range(0, jsonArrayFolders.length()).forEach(counter -> {
							...
							IntStream.range(0, profilesForObjectRights.length()).forEach(profileCounter -> {
								if (!exitFromProfileCounter
										...
										&& profilesForObjectRights.getJSONObject(profileCounter).getString("objectId")
												.equals(objId)) {
									rights = profilesForObjectRights.getJSONObject(profileCounter).getString("rights");
						...
						System.out.println("Response status code is not 200. Response code ->"+response.getStatusCode());
					}
				}
