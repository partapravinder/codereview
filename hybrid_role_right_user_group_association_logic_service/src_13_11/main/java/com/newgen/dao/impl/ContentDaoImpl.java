...
public class ContentDaoImpl implements ContentDao {
	...
	@SuppressWarnings("unchecked")
	@Override
	public Content findAndModify(String id, String updateContentParams, Long version,
			boolean ignoreCommittedFlag,String tenantId) {
		Query query = new Query();
		query.addCriteria(Criteria.where("tenantId").is(tenantId));
		if (version == null) {
			query.addCriteria(Criteria.where("_id").is(id));
	...
	public List<Content> findAllContentsByMetadata(String paramStr, String tenantId) {
		    ...
		    if (paramJson.get(key) instanceof JSONObject) {
				...
				while(constraintkeyset.hasNext()) {
				    ...
				    if(constraintkey.equalsIgnoreCase("ne")) {
						query.addCriteria(Criteria.where("metadata."+key).ne(constraintsJson.getString(constraintkey)));
				    }else if(constraintkey.equalsIgnoreCase("lte")) {
						query.addCriteria(Criteria.where("metadata."+key).lte(constraintsJson.getString(constraintkey)));
				    }else if(constraintkey.equalsIgnoreCase("lt")) {
						query.addCriteria(Criteria.where("metadata."+key).lt(constraintsJson.getString(constraintkey)));
				    }else if(constraintkey.equalsIgnoreCase("gte")) {
						query.addCriteria(Criteria.where("metadata."+key).gte(constraintsJson.getString(constraintkey)));
				    }else if(constraintkey.equalsIgnoreCase("gt")) {
						query.addCriteria(Criteria.where("metadata."+key).gt(constraintsJson.getString(constraintkey)));
				    }
				}
		    }
		    else
		...
		/*for (Map.Entry<String, String[]> entry : paramMap.entrySet()) {
			if (entry.getValue()[0].toString().contains("*")) {
				query.addCriteria(Criteria.where("metadata."+entry.getKey()).regex(toLikeRegex(entry.getValue()[0]),"i"));
				continue;
			}
			query.addCriteria(Criteria.where("metadata."+entry.getKey()).is(entry.getValue()[0]));
		}
		*/
		query.addCriteria(Criteria.where(DELETED_PARAM).ne("true"));
		query.addCriteria(Criteria.where("tenantId").is(tenantId));
