	...
	public String readUserRightsById(String id, String userId, String tenantId, String objectType)
		...
		if (objectType.equalsIgnoreCase("folder")) {
			...
			callLoggingService(tenantId, null, "CosmosDB", startTime, endTime, inOutParameters.getRequestPayloadSize(),
					inOutParameters.getResponsePayloadSize(), "GET", "UserGroupLogicService");
			...
			long startTime = System.nanoTime();
			InOutParameters inOutParameters = contentDao.findOne(id, tenantId);
			long endTime = System.nanoTime();
			callLoggingService(tenantId, null, "CosmosDB", startTime, endTime, inOutParameters.getRequestPayloadSize(),
					inOutParameters.getResponsePayloadSize(), "GET", "UserGroupLogicService");
		...
		try {
			String apiurl = groupprofileUrl + "/groupprofile?" + "groupName=Admin";

			List<GroupProfile> groupProfiles = new ArrayList<GroupProfile>();
			HttpEntity<String> request = new HttpEntity<>(mapper.writeValueAsString(groupProfiles), headers);
			apiresponse = restTemplate.exchange(apiurl, HttpMethod.GET, request, String.class);

			if (apiresponse != null) {
				if (apiresponse.getStatusCode().equals(HttpStatus.OK)) {
					JSONArray array1 = new JSONArray(apiresponse.getBody());
					if (array1.getJSONObject(0).has("userIds")) {
						JSONArray userIds1 = array1.getJSONObject(0).getJSONArray("userIds");
						for (Object object : userIds1) {
