...
public class WrapperMongoServiceImpl<T> implements WrapperMongoService<T> {
	...
	@Override
	public <T> AggregationResults<T> aggregate(Aggregation query,String collectionName, Class<T> entityClass) {
		return mongoTemplate.aggregate(query, collectionName, entityClass);
	}
