...
import com.newgen.model.ReportsDataRequest;
...
public class DataClassController extends ExceptionThrower {
	...
	@RequestMapping(method = RequestMethod.POST, value = "/reportsdatabyfield", produces = APPLICATION_JSON_VALUE)
	@ApiOperation(value = "Get Reports data by field", produces = APPLICATION_JSON_VALUE)
	@ResponseStatus(code = HttpStatus.OK)
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Get Content Status"),
			@ApiResponse(code = 500, message = "Internal Server Error"),
			@ApiResponse(code = 503, message = "Database connection failure"),
			@ApiResponse(code = 400, message = "Exception Message") })
	public ResponseEntity<ReportsDataResponse> getReportsDataByField(@RequestBody ReportsDataRequest reportsDataRequest,
		...
		String tenantId = request.getHeader("tenantId").toString();
		ReportsDataResponse reportsDataResponse = dataClassService.reportsDataByField(
				reportsDataRequest.getDataClassField(), reportsDataRequest.getMonth(), reportsDataRequest.getYear(),
				tenantId);

		if (reportsDataResponse == null) {
			return new ResponseEntity<ReportsDataResponse>(reportsDataResponse, HttpStatus.NOT_FOUND);
		}
		return new ResponseEntity<ReportsDataResponse>(reportsDataResponse, HttpStatus.OK);
	}
